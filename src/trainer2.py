...
def solve_pb(problems_test, model, path):
    ...
    for _, problem in enumerate(test_bar):
        ...
        desc = "%d rnds: tot time %.2f ms for %d cases, avg time: %.2f ms; the pred acc is %.2f, in which TP: %.2f, TN: %.2f, FN: %.2f, FP: %.2f" \
               % (26, sum(times), len(times), sum(times) * 1.0 / len(times), (TP + TN) * 1.0 / num_cases,
                  2 * TP * 1.0 / num_cases, 2 * TN * 1.0 / num_cases, 2 * FN * 1.0 / num_cases,
                  2 * FP * 1.0 / num_cases)
    print(desc)

    for _, problem in enumerate(test_bar):
        solutions, fl, fc = model.find_solutions(problem, model, path)
        target = np.array(problem.is_sat)
